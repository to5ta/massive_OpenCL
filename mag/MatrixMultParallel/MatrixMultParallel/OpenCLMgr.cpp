...
OpenCLMgr::~OpenCLMgr()
{
	...
	//Release kernels
	if (matMultPara_kernel) status = clReleaseKernel(matMultPara_kernel);
...
cl_int OpenCLMgr::init()
{
	...
	const char *filename = "/Users/mag/parallelComputing/MatrixMultParallel/MatrixMultParallel/MatMultPara_Kernel.cl";
	string sourceStr;
	...
	matMultPara_kernel = clCreateKernel(program,"matMultGlob_kernel", &status);
	CHECK_SUCCESS("Error: creating matMultGlob_kernel kernel")
