...
OpenCLMgr::~OpenCLMgr()
{
	...
	//Release kernels
	if (matMultGlob_kernel) status = clReleaseKernel(matMultGlob_kernel);
...
cl_int OpenCLMgr::init()
{
	...
	const char *filename = "/Users/mag/parallelComputing/MatrixMultGlobal/MatrixMultGlobal/MatMultGlob_Kernel.cl";
	string sourceStr;
	...
	matMultGlob_kernel = clCreateKernel(program,"matMultGlob_kernel", &status);
