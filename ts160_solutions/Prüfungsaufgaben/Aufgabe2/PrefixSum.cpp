...
void PrefixSum::plotData(cl_uint* owndata){
    int columns = 32;
    int lines   = 3;
    printf("[");
    for (int i=0; i<datalength; i++){
        ...
        if(i+1==columns*lines){
            printf("  ...\n ");
            i = datalength-1-columns*lines;
        }
    }
    printf("\n");
}
...
void PrefixSum::BlockwisePrefix(cl_uint* data, int length){
    return;
}
...
void PrefixSum::prefixSumGPU(){
    ...
    if(datalength*4>OpenCLmgr->maxMem){
        printf(ANSI_COLOR_RED);
        printf("Sorry, you want to allocate %i Bytes but the card does only support %i Bytes at once!", datalength*4, OpenCLmgr->maxMem);
        printf("Aborting....");
        printf(ANSI_COLOR_RESET);
        return;
    } else {
...
//        printf("%x", inputs[level]);
    ...
    cl_mem BufferA = clCreateBuffer(this->OpenCLmgr->context, CL_MEM_READ_ONLY, datalength*sizeof(cl_uint), NULL, NULL);

    status          = clEnqueueWriteBuffer(OpenCLmgr->commandQueue, BufferA, CL_TRUE, 0, this->datalength*sizeof(cl_uint), this->inputData, 0, NULL, NULL);
    check_error(status);
...
//    size_t global_work_size[1] = {gws_0};
    ...
    status = clEnqueueReadBuffer(OpenCLmgr->commandQueue, OutBuffer, CL_TRUE, 0, datalength*sizeof(cl_uint), inputData, 0, NULL, NULL);
    check_error(status);
